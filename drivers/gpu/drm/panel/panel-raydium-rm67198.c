...
static int rm67198_enable(struct rad_panel *panel)
{
	...
	/* Set DSI mode */
	dsi_mode = (dsi->mode_flags & MIPI_DSI_MODE_VIDEO) ? 0x08 : 0x00;
	...
	usleep_range(15000, 17000);
        ...
        /*IWG34M/IWG37M: MIPI DSI: Display Command Set to rotate the display*/
        ret = mipi_dsi_generic_write(dsi, (u8[]){ 0x36, 0xa2 }, 2);
        if (ret < 0) {
                dev_err(dev, "Failed to rotate display (%d)\n", ret);
                goto fail;
        }
