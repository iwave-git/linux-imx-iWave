...
#define    HYN_RESET_PIN  1   /* IWG34M/IWG37M: Reset: Defining touch reset gpio number for GPIO1_1 */ 
...
static unsigned int  g_irq_pin = 8; /* IWG34M/IWG37M: Interrupt: Defining interrupt gpio number for GPIO1_8 */
...
#define debug_cst(fmt,arg...)  printk(KERN_ERR" linc HYN:[LINE=%d]"fmt,__LINE__, ##arg)							
...
static void cst3xx_reset_ic(unsigned int ms)
{
	...
	/* IWG34M/IWG37M: Touch Software Reset not used */
	unsigned char buf[4];	
	buf[0] = 0xD1;	
	buf[1] = 0x0E;	 	
...
static const struct proc_ops proc_tool_debug_proc_ops = {
        .proc_read      = cst3xx_proc_read_foobar,
        .proc_write     = cst3xx_proc_write_foobar,
};
...

{
   ...
   g_update_file = proc_create(CST3XX_PROC_FILE_NAME, 0777, g_proc_dir,&proc_tool_debug_proc_ops);
...
static const struct proc_ops tp_version_proc_fops1 = {
        .proc_open      = proc_tp_version_open_cst3xx,
        .proc_read     = seq_read,
};
